...
use zenoh_node::node::Publisher;

#[derive(Debug, Snafu)]
pub enum ParquetDatafeedError {
...
impl Default for ParquetDatafeed {
    fn default() -> Self {
        Self {
...
pub struct ParquetDatafeedConfig {
    root_log_path: String,
}

impl Default for ParquetDatafeedConfig {
    fn default() -> Self {
        Self {
            root_log_path: String::new(),
        }
    }
}

#[async_trait]
impl DatafeedSpawn for ParquetDatafeed {
    async fn datafeed(self) -> Result<(), DatafeedError> {
        let parquet_config = self.parse_config(&self.config_path)?;

        let _ = self
            .publish_historical_data(&parquet_config.root_log_path)
            .await;
        ...
        for (i, df) in self.datafeeds.iter().enumerate() {
            ...
            // If tickets are enabled then gather ticks from trades.parquet
            data.0 = if df.enable_ticks {
                let mut ticks = VecDeque::new();
                let path = Path::new(&parquet_config.root_log_path)
                    .join(&df.symbol)
                    .join(&self.start_date)
                    .join(&contract)
                    .join(format!("trades.parquet"));

                let res = File::open(&path).context(FileNotFoundSnafu {
                    filename: path.clone().into_os_string().into_string().unwrap(),
                });
                ...
                if res.is_err() {
                    ...
                    if let Some(publisher) = &self.complete_publisher {
                        tracing::debug!(
                            "Resim/Backtest complete, publish datafeed complete message, id: {}",
                            self.datafeed_id
                        );
                        publisher
                    ...
                    File::open(&path).context(FileNotFoundSnafu {
                        filename: path.clone().into_os_string().into_string().unwrap(),
                    })?;
                }
                ...
                while let Some(Ok(record_batch)) = reader.next() {
                    ...
                    {
                        let ts_arr = ts.as_any().downcast_ref::<UInt64Array>().unwrap().values();
                        let p_arr = p.as_any().downcast_ref::<Float32Array>().unwrap().values();
                        let sz_arr = sz.as_any().downcast_ref::<UInt32Array>().unwrap().values();
                        let sd_arr = sd.as_any().downcast_ref::<Int8Array>().unwrap().values();
            ...
            // For each request candle stream, find corresponding parquet files for resim/backtest
            for (j, c) in df.candle_config.iter().enumerate() {
                ...
                let path = Path::new(&parquet_config.root_log_path)
                    ...
                    .join(&self.start_date)
                    .join(&contract)
                    .join(format!("ohlcv-{}.parquet", c.period_s));
                ...
                while let Some(Ok(record_batch)) = reader.next() {
                        ...
                        (timestamp_ns, open, high, low, close, volume)
                    {
                        ...
                        let c_arr = cl.as_any().downcast_ref::<Float32Array>().unwrap().values();
        ...
        // Populate queue for a simulated datafeed. Loop through number of messages stored in datafeed buffer df_data.
        ...
        if let Some(publisher) = &self.complete_publisher {
            tracing::debug!(
                "Datafeed complete, publish datafeed complete message, id: {}",
                self.datafeed_id
            );
            publisher
                .publish(DatafeedComplete {
...
impl DatafeedSetup for ParquetDatafeed {
...
impl ParquetDatafeed {
    async fn publish_historical_data(&self, log_path: &str) -> Result<(), ParquetDatafeedError> {
        for df in &self.datafeeds {
            for c in &df.candle_config {
                let mut historical_data = HistoricalData::default();
                let mut date_strings = VecDeque::new();
                ...
                while num_days > 0 && iter_count < 7 {
                    ...
                    if let Some(sub_dt) = dt.checked_sub_days(Days::new(1)) {
                        ...
                        // log found so move onto the next day
                        if path.exists() {
                            num_days -= 1;
                            // push front so dates are in order
                            date_strings.push_front(path);
                            curr_date = sub_str;
                            iter_count = 0;
                        }
                    }
                ...
                for ds in &date_strings {
                    let file = File::open(ds).context(FileNotFoundSnafu {
                        filename: ds.clone().into_os_string().into_string().unwrap(),
                    })?;
                    ...
                    while let Some(Ok(record_batch)) = reader.next() {
                            ...
                            (timestamp_ns, open, high, low, close, volume)
                        {
                            ...
                            tracing::error!(
                                "Error reading parquet file for date: {}",
                                ds.to_str().unwrap()
                            );
                        }
                    }
    ...
    fn parse_config(&self, config_path: &str) -> Result<ParquetDatafeedConfig, DatafeedError> {
        let f = std::fs::read_to_string(&config_path).context(ReadFileToStringSnafu {
            filename: config_path,
        })?;
        let config: ParquetDatafeedConfig = serde_yaml::from_str(&f).context(SerdeYamlSnafu {
            filename: config_path,
        })?;
    ...
    // Get futures contract symbol as it changes based on the date
    fn get_contract_symbol(symbol: &str, dt: &NaiveDate) -> Result<String, ParquetDatafeedError> {
