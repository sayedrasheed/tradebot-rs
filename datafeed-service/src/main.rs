...
async fn main() -> Result<(), ExitError> {
        ...
        .with(
            stdout_log
                ...
                .with_filter(tracing_subscriber::filter::filter_fn(|metadata| {
                    metadata.target().starts_with("datafeed") // filter for only datafeed service
                })),
        )
        .init();
    ...
    tracing::info!("Starting Datafeed Service");
    let service = DatafeedService::new(&config).await?;

    service.join().await?;
    tracing::info!("Datafeed Service has exited");
